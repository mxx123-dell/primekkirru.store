...
function sendMessDichVu($my_text){
    $CMSNT = new DB;
    if($my_text != ''){
        if($CMSNT->site('discord_notification') == 'discord'){
            return sendMessDiscord($my_text);
        }
        return false;
    }
    return false;
...
function sendMessDiscord($my_text, $webhook_url = ''){
    $CMSNT = new DB;
    if($webhook_url == ''){
        $webhook_url = $CMSNT->site('discord_webhook');
    }
    if($webhook_url != ''){
        $data = array('content' => $my_text);
        $options = array(
            'http' => array(
